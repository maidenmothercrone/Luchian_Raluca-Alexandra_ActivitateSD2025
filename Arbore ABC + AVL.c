...
int determinaNrNoduri(Nod* rad) {
	if (rad == NULL) {
		return 0;
	}
	else {
		return 1 + determinaNrNoduri(rad->stanga) + determinaNrNoduri(rad->dreapta);
	}
}
...
int main() {
	...
	printf("Masina cu id 10 este:\n");
	afisareMasina(getMasinaById(rad, 10));

	printf("Inaltime arbore: %d", calculeazaInaltimeArbore(rad));

	printf("Numar noduri arbore: %d", determinaNrNoduri(rad));
